...
class CrawlCSVRequest(BaseModel):
    ...
    max_threads: int = Field(default=20, ge=1, le=100, description="Maximum number of concurrent sites to crawl (max 100).")

def log_resource_usage():
    """Logs current CPU, memory, and open connections."""
    ...
    try:
        ...
        connections = "N/A"
    logger.info(f"Resource Usage: CPU={cpu_percent}%, Memory={mem_info.rss / 1024 / 1024:.2f}MB, Connections={connections}")
        ...
        try:
            async with session.get(sitemap_url, timeout=10) as response:
                if response.status != 200:
                    ...
                    return [(start_url, "")]  # Fallback to start_url
...
async def crawl_website_single_site(
    ...
    """Crawl a single website, processing only modified URLs."""
    crawled_urls = set()
    queued_urls = set()
    ...
    try:
        ...
        logger.error(f"Critical error fetching sitemap for {start_url}: {e}")
        results["failed"].append({"url": start_url, "error": f"Sitemap fetch error: {e}"})
        return results
    ...
    logger.info(f"Starting crawl for {start_url} from {initial_url} with {len(urls_to_crawl)} modified URLs")
    ...
    async def crawl_page():
            ...
            try:
                ...
                try:
                    ...
                    if current_domain != crawl_start_domain:
                        logger.info(f"Skipping external URL: {current_url}")
                        crawled_urls.add(current_url)
                        crawl_queue.task_done()
                        continue
                except Exception as e:
                    logger.error(f"Error parsing domain for URL {current_url}: {e}")
                    crawled_urls.add(current_url)
                    crawl_queue.task_done()
                    continue
                ...
                try:
                    ...
                    results["failed"].append({"url": current_url, "error": f"Network error: {e}"})
                    crawl_queue.task_done()
                    continue
        ...
        try:
            ...
            logger.error(f"Error saving progress for {start_url}: {e}")
        ...
        try:
            ...
            logger.error(f"Error updating file_info_list.csv: {e}")
    ...
    try:
        ...
        logger.error(f"Error in worker tasks for {start_url}: {e}")
    finally:
...
async def crawl_from_csv(csv_content: str, output_dir: str, max_concurrency: int, max_depth: int, max_threads: int):
    """Crawls URLs from a CSV content string."""
    try:
...
async def crawl_csv_upload_endpoint(
    ...
    max_threads: int = Form(default=20, ge=1)
):
...
def run_daemon():
    ...
    csv_path = "C:/Users/victo/Desktop/multithreads_crawling/urls.csv"
    ...
    async def daily_crawl():
        ...
        try:
            with open(csv_path, "r", encoding="utf-8") as f:
                csv_content = f.read()
            await crawl_from_csv(csv_content, output_dir, max_concurrency, max_depth, max_threads)
        except Exception as e:
