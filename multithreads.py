...
class CrawlCSVRequest(BaseModel):
    ...
    max_concurrency_per_site: int = Field(default=20, ge=1, le=50, description="Maximum concurrent requests *per site being crawled* (max 50).")
    max_depth: int = Field(default=2, ge=0, description="Maximum depth to crawl from each starting URL in the CSV.")
    max_threads: int = Field(default=50, ge=1, le=100, description="Maximum number of concurrent sites to crawl (max 100).")
...
async def crawl_website_single_site(
    ...
    logger.info(f"Starting crawl for: {start_url} with max_depth={max_depth}, max_concurrency={max_concurrency}")
...
def run_crawl_in_thread(start_url: str, output_dir: str, max_concurrency: int, max_depth: int) -> Dict[str, Any]:
    """Run crawl_website_single_site in a separate thread."""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        result = loop.run_until_complete(
            crawl_website_single_site(start_url, output_dir, max_concurrency, max_depth)
        )
        return result
    except Exception as e:
    ...
    try:
        ...
        for pf in progress_files:
            with open(os.path.join(output_dir, pf), "r", encoding="utf-8") as f:
                ...
                if progress["results"].get("success"):
                    completed_urls.add(progress["results"]["initial_url"])
